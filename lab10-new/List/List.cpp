...
void List<T>::insertEnd(const T& obj) {
    if(isEmpty()) headPtr = new Node<T>(obj);
    else {
...
void List<T>::removeAt(int pos) throw (std::runtime_error) {
    if(pos < 1 || pos > getLength()) {
        throw(std::runtime_error("Invalid deletion index for linked list. Method: 'removeAt'.\n"));
    }
    else if(pos == 1) {
...
template class List<int>; // instantiate type parameter as int upon complilation
template class List<WEdge>;
template class List<AdjEntry>;
