...
template <typename T>
DSetCollection<T>::DSetCollection(int size, CompressMode m) {
    mode = m;
    max_size = size;
    n = 0;
...
const T& DSetCollection<T>::findRepr(const T& x) {
    ...
    if(ptr == nullptr) throw x;
        ...
        if(mode == AUTO) ptr->compress();
        return ptr->repr()->item;
    }
}
...
template <typename T>
int DSetCollection<T>::size() const {
    return n;
}

template <typename T>
DNode<T>** DSetCollection<T>::nodes() {
    return ptrs;
}

template class DSetCollection<int>;
template class DSetCollection<WEdge>;